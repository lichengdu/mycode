...
public class OrganizationDaoImpl implements OrganizationDao {
    ...
    public Organization createOrganization(final Organization organization) {
        final String sql = "insert into shiro.sys_organization( id,name, parent_id, parent_ids, available) values(?,?,?,?,?)";
        ...
        jdbcTemplate.update(new PreparedStatementCreator() {
            public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                ...
                int count = 1;
                psst.setInt(count++, getMaxId("shiro.sys_organization"));
    ...
    public Organization updateOrganization(Organization organization) {
        final String sql = "update shiro.sys_organization set name=?, parent_id=?, parent_ids=?, available=? where id=?";
        jdbcTemplate.update(
                sql,
    ...
    public void deleteOrganization(Long organizationId) {
        ...
        final String deleteSelfSql = "delete from shiro.sys_organization where id=?";
        jdbcTemplate.update(deleteSelfSql, organizationId);
        final String deleteDescendantsSql = "delete from shiro.sys_organization where parent_ids like ?";
        jdbcTemplate.update(deleteDescendantsSql, organization.makeSelfAsParentIds() + "%");
    }


    public Organization findOne(Long organizationId) {
        final String sql = "select id, name, parent_id, parent_ids, available from shiro.sys_organization where id=?";
    ...
    public List<Organization> findAll() {
        final String sql = "select id, name, parent_id, parent_ids, available from shiro.sys_organization";
        return jdbcTemplate.query(sql, new BeanPropertyRowMapper(Organization.class));
    }

    public List<Organization> findAllWithExclude(Organization excludeOraganization) {
        //TODO 改成not exists 利用索引
        final String sql = "select id, name, parent_id, parent_ids, available from shiro.sys_organization where id!=? and parent_ids not like ?";
    ...
    public void move(Organization source, Organization target) {
        String moveSourceSql = "update shiro.sys_organization set parent_id=?,parent_ids=? where id=?";
        jdbcTemplate.update(moveSourceSql, target.getId(), target.getParentIds(), source.getId());
        String moveSourceDescendantsSql = "update shiro.sys_organization set parent_ids=concat(?, substring(parent_ids, length(?))) where parent_ids like ?";
