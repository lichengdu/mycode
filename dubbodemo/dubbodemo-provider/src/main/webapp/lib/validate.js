Validator = {
    Require: {expression: /.+/, msg: "该字段不能为空"},
    Email: {expression: /^\w+([-+.]\w+)*@\w+([-.]\\w+)*\.\w+([-.]\w+)*$/, msg: "Email格式不正确"},
    Mobile: {expression: /^(1[3|5|8]{1}\d{9})$/, msg: "手机格式不正确"},
    TelePhone: {expression: /^(((0\d{2,3}-)?\d{7,8}(-\d{4})?))$/, msg: "电话号码格式不正确"},
    IdCard: {expression: /^\d{15}(\d{2}[A-Za-z0-9])?$/, msg: "身份证号码格式不正确"},
    Number: {expression: /^-?(\d+|[1-9]\d*\.\d+|0\.\d*[1-9]\d*|0?\.0+|0)$/, msg: "仅支持数字"},
    English: {expression: /^[A-Za-z]+$/, msg: "仅支持英文字符"},
    Chinese: {expression: /^[\u0391-\uFFE5]+$/, msg: "仅支持中文字符"},
    URL: {
        expression: /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
        msg: "URL地址格式不正确"
    },
    Regex: {msg: "格式不正确"},
    ...
    SafeString: {expression: "this.IsSafe(value)", msg: "仅支持中文字符"},
    ...
    Validate: function (theForm, mode) {
        ...
        for (var i = 0; i < count; i++) {
            with (obj.elements[i]) {
                ...
                var msg = getAttribute("msg") ? getAttribute("msg") : this[_dataType].msg;
                msg = msg || "invalid";
