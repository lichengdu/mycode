...
public class ResourceDaoImpl implements ResourceDao {
	...
	public Resource createResource(final Resource resource) {
		final String sql = "insert into shiro.sys_resource(id,name, type, url, permission, parent_id, parent_ids, available) values(?,?,?,?,?,?,?,?)";
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				...
				int count = 1;
				psst.setInt(count++, getMaxId("shiro.sys_resource"));
	...
	public Resource updateResource(Resource resource) {
		final String sql = "update shiro.sys_resource set name=?, type=?, url=?, permission=?, parent_id=?, parent_ids=?, available=? where id=?";
	...
	public void deleteResource(Long resourceId) {
		...
		final String deleteSelfSql = "delete from shiro.sys_resource where id=?";
		jdbcTemplate.update(deleteSelfSql, resourceId);
		final String deleteDescendantsSql = "delete from shiro.sys_resource where parent_ids like ?";
		jdbcTemplate.update(deleteDescendantsSql, resource.makeSelfAsParentIds() + "%");
	}

	public Resource findOne(Long resourceId) {
		final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available from shiro.sys_resource where id=?";
	...
	public List<Resource> findAll() {
		final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available from shiro.sys_resource order by concat(parent_ids, id) asc";
